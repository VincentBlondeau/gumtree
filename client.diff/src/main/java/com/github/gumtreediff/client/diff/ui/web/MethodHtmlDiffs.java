...
public final class MethodHtmlDiffs {
    ...
    public MethodHtmlDiffs(File fSrc, File fDst, TreeContext src, TreeContext dst, Matcher matcher) {
        this.fSrc = fSrc;
        this.fDst = fDst;
        this.src = src;
        this.dst = dst;
    ...
    public void produce() throws IOException {
        ...
        for (ITree t : src.getRoot().getTrees()) {
            if (c.getSrcMvTrees().contains(t)) {
                ...
                ltags.addTags(t.getPos(), "move"+ tooltip(src, t), t.getEndPos(), "");
            }
            if (c.getSrcUpdTrees().contains(t)) {
                mappingIds.put(mappings.getDst(t).getId(), mId);
                //ltags.addStartTag(t.getPos(), String.format(ID_SPAN, uId++));
                ltags.addTags(t.getPos(), "update" +  tooltip(src, t), t.getEndPos(), "");
            ...
            if (c.getSrcDelTrees().contains(t)) {
               // ltags.addStartTag(t.getPos(), String.format(ID_SPAN, uId++));
                ltags.addTags(t.getPos(),"delete" + tooltip(src, t), t.getEndPos(), "");
            }
           /*todo if (c.getDstAddTrees().contains(t)) {
                ltags.addStartTag(t.getPos(), String.format(ID_SPAN, uId++));
